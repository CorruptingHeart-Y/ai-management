...
|---|---|---|---|---|---|---|---|
| US01 | 查看项目成员 | Must | 查看预置成员；展示姓名和分工；成员 id 唯一；为 US04 提供负责人来源 | Member 数据契约、共用数据、成员展示及异常处理、专项与联调测试 | 彭佳成 | `feature/us01-members` | 待人工审查与集成验收 |
...
US04 和 US05 的代码及关键测试已经进入 `develop`，但人工 diff 审查、演示证据和最终 DoD 验收仍未完成，因此暂不标记为 Done。US01 在基线 `6906e49` 上完成开发和自动验证，尚未由 DRI 合入 develop，暂不标记 Done。
